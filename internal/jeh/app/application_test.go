// File: internal/jeh/app/application_test.go (application_test.go) // Date: 2026-09-17 // Version/Status: 1.0 / Graduated frozen JEH behavior // Product/Component: JEH-HOHO / Frozen JEH Engine // Purpose: Preserve the file's existing DSE_JEH implementation or regression. // Origin: DSE_JEH_Lab/internal/jeh/app/application_test.go, Slice 0 fingerprint // Invariants: Governed JEH/Capital/runtime semantics unchanged // Non-responsibilities: No Fin transport and no public product contract.
...
import (
	...
	"jeh-hoho/internal/jeh/services"
)
...
func (writer *recordingTraceWriter) Write(_ context.Context, record execution.TraceRecord) (bson.ObjectID, error) {
	if writer.err != nil {
		return bson.NilObjectID, writer.err
	}
...
func TestPersistentHostObservationAndOneBarPipeline(t *testing.T) {
	...
	if len(services) != 3 || services["dsejeh.v1.RuntimeOperationsService"].Methods == nil || services["dsejeh.v1.RuntimeEvidenceService"].Methods == nil || services["jeh_hoho.v1.ProductEvidenceService"].Methods == nil {
		t.Fatalf("externally registered services = %v", services)
	}
	for serviceName := range services {
		if serviceName != "dsejeh.v1.RuntimeOperationsService" && serviceName != "dsejeh.v1.RuntimeEvidenceService" && serviceName != "jeh_hoho.v1.ProductEvidenceService" {
...
func TestProductLiveStreamFollowsCommittedCurrentRuntimeCapital(t *testing.T) {
	source := &idleSource{started: make(chan struct{})}
	traceWriter := &recordingTraceWriter{}
	reservoirWriter := &recordingReservoirWriter{}
	application, err := New(config.Config{
		Mode: dsejehv1.RuntimeMode_RUNTIME_MODE_ONLINE, GRPCAddress: "source:50051", ServerAddress: "127.0.0.1:0",
		StatusInterval: time.Hour, RemainRunning: true, PipelineRunID: "pipeline-live", CollectionRunID: "collection-live",
		RunType: execution.RunTypeA, StartingCapital: 100_000, AllocationPct: 1,
	}, Options{Source: source, TraceWriter: traceWriter, ReservoirWriter: reservoirWriter})
	if err != nil {
		t.Fatal(err)
	}
	...
	case <-source.started:
	case <-time.After(2 * time.Second):
		t.Fatal("DSE_JEH runtime did not start")
	}
	defer func() {
		reservoirWriter.err = nil
		stopRuntime()
		select {
		case runErr := <-runDone:
			if runErr != nil {
	...
	defer cancelStream()
	stream, err := jehhohov1.NewProductEvidenceServiceClient(connection).SubscribeEvidence(streamContext, &jehhohov1.SubscribeEvidenceRequest{})
	if err != nil {
		t.Fatal(err)
	}
	time.Sleep(20 * time.Millisecond)

	entity := &pipelineEntity{account: mustAccount(t, 100_000)}
	execute := func(sequence uint64, action dsejehv1.GovernedExecutionAction, cause string) error {
		bar := testBar(sequence)
		bar.Close = 10
		result := &dsejehv1.DynamicExecutionResult{
			ResultId:           fmt.Sprintf("result-%d", sequence),
			StateActionOutcome: &dsejehv1.StateActionOutcome{ActionOutcomeId: fmt.Sprintf("outcome-%d", sequence)},
		}
		return application.pipeline.execute(context.Background(), bar, result, entity, action, 10, cause)
	}
	if err := execute(65, dsejehv1.GovernedExecutionAction_GOVERNED_EXECUTION_ACTION_ALLOCATE, "HOP_ON_ALLOCATE"); err != nil {
		t.Fatal(err)
	}
	if len(reservoirWriter.events) != 2 {
		t.Fatalf("OUTFLOW delivered before persistence: writes=%d", len(reservoirWriter.events))
	}
	first, err := stream.Recv()
	if err != nil {
		t.Fatal(err)
	}
	if event := first.GetEvidence().GetCapitalReservoir(); event.GetPipelineRunId() != "pipeline-live" || event.GetEventSequence() != 2 || event.GetEventType() != jehhohov1.CapitalReservoirEventType_CAPITAL_RESERVOIR_EVENT_TYPE_OUTFLOW {
		t.Fatalf("first LIVE Capital event = %+v", event)
	}
	if first.GetEvidence().GetProductIdentity().GetProductRunId() != application.runtimeID {
		t.Fatalf("LIVE product run = %q, want current runtime %q", first.GetEvidence().GetProductIdentity().GetProductRunId(), application.runtimeID)
	}

	if err := execute(66, dsejehv1.GovernedExecutionAction_GOVERNED_EXECUTION_ACTION_LIQUIDATE, "HOP_OFF_LIQUIDATE"); err != nil {
		t.Fatal(err)
	}
	if len(reservoirWriter.events) != 3 {
		t.Fatalf("INFLOW delivered before persistence: writes=%d", len(reservoirWriter.events))
	}
	second, err := stream.Recv()
	if err != nil {
		t.Fatal(err)
	}
	if event := second.GetEvidence().GetCapitalReservoir(); event.GetEventSequence() != 3 || event.GetEventType() != jehhohov1.CapitalReservoirEventType_CAPITAL_RESERVOIR_EVENT_TYPE_INFLOW {
		t.Fatalf("second LIVE Capital event = %+v", event)
	}

	reservoirWriter.err = errors.New("capital persistence failed")
	if err := execute(67, dsejehv1.GovernedExecutionAction_GOVERNED_EXECUTION_ACTION_ALLOCATE, "HOP_ON_ALLOCATE"); !errors.Is(err, reservoirWriter.err) {
		t.Fatalf("failed persistence error = %v, want %v", err, reservoirWriter.err)
	}
	received := make(chan *jehhohov1.SubscribeEvidenceResponse, 1)
	go func() {
	...
	case response := <-received:
		t.Fatalf("uncommitted Capital event reached LIVE: %+v", response.GetEvidence().GetCapitalReservoir())
...
func TestTracePersistenceFailurePreventsCapitalPersistence(t *testing.T) {
	traceErr := fmt.Errorf("trace persistence failed")
	traceWriter := &recordingTraceWriter{err: traceErr}
	reservoirWriter := &recordingReservoirWriter{}
	reservoir, err := execution.NewCapitalReservoir("pipeline-a", "source-a", execution.RunTypeA, 30, 100_000, reservoirWriter)
	if err != nil {
		t.Fatal(err)
	...
	pipeline := &Pipeline{
		runtimeID: "runtime-a", bus: evidence.NewBus(), traceWriter: traceWriter, executor: execution.MockExecutor{},
		parameters: execution.RunParameters{StartingCapital: 100_000, AllocationPct: 1}, pipelineRunID: "pipeline-a",
		runType: execution.RunTypeA, collectionRunID: "source-a", reservoir: reservoir, now: time.Now,
	}
	entity := &pipelineEntity{account: mustAccount(t, 100_000)}
	result := &dsejehv1.DynamicExecutionResult{ResultId: "result-a", StateActionOutcome: &dsejehv1.StateActionOutcome{ActionOutcomeId: "outcome-a"}}
	err = pipeline.execute(context.Background(), testBar(65), result, entity, dsejehv1.GovernedExecutionAction_GOVERNED_EXECUTION_ACTION_ALLOCATE, 10, "HOP_ON_ALLOCATE")
	if !errors.Is(err, traceErr) {
		t.Fatalf("execute error = %v, want %v", err, traceErr)
	}
	if len(reservoirWriter.events) != 0 {
		t.Fatalf("Capital persisted after trace failure: %+v", reservoirWriter.events)
	}
}
