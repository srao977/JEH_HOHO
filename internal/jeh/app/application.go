// File: internal/jeh/app/application.go (application.go) // Date: 2026-09-17 // Version/Status: 1.0 / Graduated frozen JEH behavior // Product/Component: JEH-HOHO / Frozen JEH Engine // Purpose: Preserve the file's existing DSE_JEH implementation or regression. // Origin: DSE_JEH_Lab/internal/jeh/app/application.go, Slice 0 fingerprint // Invariants: Governed JEH/Capital/runtime semantics unchanged // Non-responsibilities: No Fin transport and no public product contract.
...
import (
	...
	productoutput "jeh-hoho/internal/evidence"
	...
	runtimeapp "jeh-hoho/internal/jeh/runtime"
...
type Options struct {
	...
	ProductOperations jehhohov1.ProductOperationsServiceServer
}
...
func New(cfg config.Config, options Options) (*Application, error) {
	...
	jehhohov1.RegisterProductEvidenceServiceServer(application.server, productoutput.NewGateway(&jehhohov1.ProductIdentity{
		ProductRunId:  runtimeID,
		StartedUnixMs: started.UnixMilli(),
	}, bus))
	if options.ProductOperations != nil {
		jehhohov1.RegisterProductOperationsServiceServer(application.server, options.ProductOperations)
	}
...
// RuntimeID returns the existing DSE_JEH runtime identity used by product streams.
func (application *Application) RuntimeID() string { return application.runtimeID }

// RegisterProductOperations adds the controller-owned read-only status service before Run.
func (application *Application) RegisterProductOperations(service jehhohov1.ProductOperationsServiceServer) {
	jehhohov1.RegisterProductOperationsServiceServer(application.server, service)
}
