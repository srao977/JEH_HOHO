// File: internal/jeh/execution/capital_reservoir_test.go (capital_reservoir_test.go) // Date: 2026-09-17 // Version/Status: 1.0 / Graduated frozen JEH behavior // Product/Component: JEH-HOHO / Frozen JEH Engine // Purpose: Preserve the file's existing DSE_JEH implementation or regression. // Origin: DSE_JEH_Lab/internal/jeh/execution/capital_reservoir_test.go, Slice 0 fingerprint // Invariants: Governed JEH/Capital/runtime semantics unchanged // Non-responsibilities: No Fin transport and no public product contract.
...
func TestCapitalReservoirPersistenceFailureDoesNotMutateStateOrSequence(t *testing.T) {
	writer := &recordingReservoirWriter{}
	reservoir, err := NewCapitalReservoir("pipeline", "source", RunTypeA, 30, 100_000, writer)
	if err != nil {
		t.Fatal(err)
	...
	writer.err = errors.New("capital persistence failed")
	buy := reservoirInstruction("buy", dsejehv1.GovernedExecutionAction_GOVERNED_EXECUTION_ACTION_ALLOCATE, "10")
	if _, err := reservoir.RecordExecution(context.Background(), buy, reservoirExecutionEvent("buy", "10", "5", when), CapitalReservoirCauseHopOn, bson.NewObjectID(), when, when); !errors.Is(err, writer.err) {
		t.Fatalf("RecordExecution error = %v, want %v", err, writer.err)
	}
	writer.err = nil
	event, err := reservoir.RecordExecution(context.Background(), buy, reservoirExecutionEvent("buy", "10", "5", when), CapitalReservoirCauseHopOn, bson.NewObjectID(), when, when)
	if err != nil {
		t.Fatal(err)
	}
	if event.EventSequence != 2 || event.ReservoirBefore != 3_000_000 || event.ReservoirAfter != 2_999_950 || event.ActiveQuantityAfter == nil || *event.ActiveQuantityAfter != 10 {
		t.Fatalf("state changed across persistence failure: %+v", event)
	}
}
