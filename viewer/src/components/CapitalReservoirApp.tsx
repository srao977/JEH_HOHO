...
export default function CapitalReservoirApp() {
  ...
  const [status, setStatus] = useState("Connecting to JEH-HOHO live stream");
  ...
  function changeMode(nextMode: Mode) {
    ...
    setError("");
    setStatus(nextMode === "LIVE" ? "Connecting to JEH-HOHO live stream" : "Loading ordered reservoir events");
    setMode(nextMode);
  }

  return <main className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-mark"><WalletCards size={20} /></div><div><h1>Capital Reservoir</h1><p>JEH-HOHO common-capital viewer</p></div></div><div className="top-status"><span className={`signal ${error ? "fault" : ""}`} /><div><strong>{mode} · READ ONLY</strong><span>{status}</span></div></div></header>
    <section className="control-band"><div className="segmented" aria-label="Viewer mode">{(["LIVE", "REPLAY"] as Mode[]).map((value) => <button type="button" key={value} className={mode === value ? "active" : ""} onClick={() => changeMode(value)}>{value}</button>)}</div>
      ...
      {mode === "LIVE" && <button className="icon-command" title="Reconnect live stream" aria-label="Reconnect live stream" onClick={() => { setStatus("Reconnecting to JEH-HOHO live stream"); setLiveGeneration((value) => value + 1); }}><RefreshCw /></button>}
    </section>
