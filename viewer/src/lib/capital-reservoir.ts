// JEH_HOHO product viewer; graduated from Capital_Reservoir_Viewer on 2026-09-17.
...
export function normalizeReservoirEvent(input: unknown): CapitalReservoirEvent {
  ...
  const eventType = enumValue(field(record, "eventType", "event_type"), "CAPITAL_RESERVOIR_EVENT_TYPE_") as ReservoirEventType;
  ...
  return {
    eventId: text(field(record, "eventId", "event_id")), pipelineRunId, collectionRunId, runType, eventSequence,
    symbol: text(field(record, "symbol", "symbol")), eventType,
    ...
    executionEventTime: iso(field(record, "executionEventUnixMs", "execution_event_time")), processedAt,
    quantity: numberValue(field(record, "quantity", "quantity")), executionPrice: numberValue(field(record, "executionPrice", "execution_price")),
    signedFlowAmount: numberValue(field(record, "signedFlowAmount", "signed_flow_amount")), initialReservoir: numberValue(field(record, "initialReservoir", "initial_reservoir")),
    symbolCount: numberValue(field(record, "symbolCount", "symbol_count")), initialSymbolCapital: numberValue(field(record, "initialSymbolCapital", "initial_symbol_capital")),
    reservoirBefore: numberValue(field(record, "reservoirBefore", "reservoir_before")) ?? 0, reservoirAfter: numberValue(field(record, "reservoirAfter", "reservoir_after")) ?? 0,
    activeQuantityAfter: numberValue(field(record, "activeQuantityAfter", "active_quantity_after")), reservoirCash: numberValue(field(record, "reservoirCash", "reservoir_cash")),
    totalDeployedAfter: numberValue(field(record, "totalDeployedAfter", "total_deployed_after")), totalMarkedCapitalAfter: numberValue(field(record, "totalMarkedCapitalAfter", "total_marked_capital_after")),
    realizedPnl: numberValue(field(record, "realizedPnl", "realized_pnl")), unrealizedPnl: numberValue(field(record, "unrealizedPnl", "unrealized_pnl")),
    totalPnl: numberValue(field(record, "totalPnl", "total_pnl")), activePositions: numberValue(field(record, "activePositions", "active_positions")),
  };
}
...
export function deriveCapitalSummary(events: CapitalReservoirEvent[], symbols: string[] = []): CapitalSummary {
  ...
  const participatingSymbols = new Set(ordered.filter((event) => event.symbol && (event.eventType === "OUTFLOW" || event.eventType === "INFLOW")).map((event) => event.symbol)).size;
  return {
    initialCapital, reservoirCash, deployedMarkedCapital: end?.totalDeployedAfter ?? null,
    totalMarkedCapital: end?.totalMarkedCapitalAfter ?? null, realizedPnl: end?.realizedPnl ?? null,
    unrealizedPnl: end?.unrealizedPnl ?? null, totalPnl: end?.totalPnl ?? null,
    activeSymbols: pipeStates.filter((pipe) => pipe.activeQuantity > 0).length, participatingSymbols,
    utilizationPct: initialCapital && end?.totalDeployedAfter !== null && end?.totalDeployedAfter !== undefined ? end.totalDeployedAfter / initialCapital * 100 : null,
...
export function derivePipeStates(events: CapitalReservoirEvent[], symbols: string[] = []): PipeState[] {
  ...
  for (const symbol of symbols) states.set(symbol, emptyPipeState(symbol));
  for (const event of mergeReservoirEvents(events)) {
    ...
    if (event.eventType === "OUTFLOW") {
      current.currentCycle = {
        entryCause: "HOP_ON", quantity: event.quantity ?? current.activeQuantity,
        capitalDeployed: Math.abs(event.signedFlowAmount ?? 0), entryPrice: event.executionPrice,
        entryEventSequence: event.eventSequence, triggerTime: event.triggerEventTime,
        entryExecutionTime: event.executionEventTime, capitalReturned: null, realizedPnl: null,
        exitCause: null, exitEventSequence: null, exitExecutionTime: null,
      };
    } else if (current.currentCycle) {
      const capitalReturned = Math.abs(event.signedFlowAmount ?? 0);
      current.completedCycles.push({
        ...current.currentCycle, capitalReturned, realizedPnl: capitalReturned - current.currentCycle.capitalDeployed,
        exitCause: event.cause === "HOP_OFF" || event.cause === "SAFETY_LIQUIDATION" ? event.cause : null,
        exitEventSequence: event.eventSequence, exitExecutionTime: event.executionEventTime,
      });
...
function emptyPipeState(symbol: string): PipeState {
  return { symbol, activeQuantity: 0, capitalDeployed: null, capitalReturned: null, realizedPnl: null, lastExecutionPrice: null, cumulativeReservoirFlow: 0, lastCause: null, lastEventSequence: 0, currentCycle: null, completedCycles: [] };
}