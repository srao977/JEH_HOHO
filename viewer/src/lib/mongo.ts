...
function config() {
  return {
    uri: process.env.JEH_HOHO_MONGO_URI?.trim() || "mongodb://127.0.0.1:27017",
    database: process.env.JEH_HOHO_MONGO_DB?.trim() || "bar_sequence_db",
    collection: process.env.JEH_HOHO_MONGO_COLLECTION?.trim() || "capital_reservoir_events",
  };
}
